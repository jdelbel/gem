...
ss <- read_csv(here("files", "ss_chl.csv"))

ic <- read_csv(here("files", "sensor_intercomp.csv"))

ec <- read_csv(here("files", "extract_comp.csv"))

cc <- read_csv(here("files", "cali_comp.csv"))
...
ic %>% 
  ggplot(aes(x = Instrument, y = ug_l, fill = Instrument)) +
  geom_boxplot(alpha = 0.5, size = 1) +
  ggpubr::stat_compare_means(method = "t.test", label.x = 0.65, label.y = 1,
                             size = 9) +
  geom_point(size = 5, pch = 21,
             position = position_jitterdodge(jitter.width = 0)) +
  ylim(0, 16) +
...
ggsave(here("figures", "intercomp.png"),
        width = 8, height = 6, dpi = 300)
```
...
```{r}
ggplot() +
  geom_smooth(data = c, aes(x = turb-0.5, y = depth, fill = as.factor(castpk),
                           color = as.factor(castpk)),
              orientation = "y", method = "loess", se = F,
              alpha = 0.4, size = 2) +
  geom_point(data = c, aes(x = turb-0.5, y = depth),
              alpha = 0.4, size = 2, fill = "white", pch = 21) +
  geom_point(data = c, aes(x = turb_ftu, y = depth, color = "AQUAFLUOR"),
              pch = 8, size = 4, stroke = 1.5) +
  scale_color_manual(values = c("#D00000", "#ffba08", "#3f88c5"),
...
ggsave(here("figures", "profiles_turb.png"),
...
ec_long <- ec %>%
  select(-avg) %>% 
  pivot_longer(c(af_1:af_6), names_to = "rep", values_to = "af_chl") %>% 
  group_by(site, depth) %>% 
  mutate(mean_af_chl = mean(af_chl, na.rm = T),
         sdev_af_chl = sd(af_chl, na.rm = T)) %>% 
  ungroup()

formula_2 <- y~x

f1 <- ggplot() +
  geom_smooth(data = ec_long, aes(x = chl, y = mean_af_chl),
              method = "lm", color = "black", fill = "black",
              formula = y ~ x,
              fullrange = T, se = T, size = 1, alpha = 0.3) +
  geom_point(data = ec_long, aes(x = chl, y = mean_af_chl),
              alpha = 0.8, size = 4,  stroke = 1.5) +
  stat_poly_eq(data = ec_long, aes(x = chl, y = mean_af_chl,
               label = paste(..eq.label.., ..rr.label.., sep = "~~")),
               formula = formula_2, parse = TRUE, size = 8,
               color = "black", label.y.npc = 0.99, label.x.npc = 0.04) +
  geom_abline(slope = 1, linetype = "dashed") +
  ggsci::scale_fill_aaas() +
  ggsci::scale_color_aaas() +
  lims(x = c(0, 110),
       y = c(0, 110)) +
  labs(x = expression("Chl"[EXTRACTED] ~ "(µg/L)"),
       y = expression("Chl"[AQUAFLUOR] ~ "(µg/L, Chl)"),
       fill = "Depth",
       color = "Depth") +
  theme_bw() +
  theme(text = element_text(size = 32),
...
# ggsave(here("figures", "scatter_extracted_comp.png"),
#         width = 8, height = 6, dpi = 300) 
```
...
f2 <- cc_long %>% 
  filter(type == "e") %>% 
  ggplot() +
  geom_smooth(aes(x = s_1, y = mean_af_chl),
              method = "lm", color = "black", fill = "black",
              formula = y ~ x,
              fullrange = T, se = T, size = 1, alpha = 0.3) +
  geom_point(aes(x = s_1, y = mean_af_chl),
              alpha = 0.8, size = 4, stroke = 1.5) +
  stat_poly_eq(aes(x = s_1, y = mean_af_chl,
               label = paste(..eq.label.., ..rr.label.., sep = "~~")),
               formula = formula_2, parse = TRUE, size = 8,
               color = "black", label.y.npc = 0.99, label.x.npc = 0.04) +
  geom_abline(slope = 1, linetype = "dashed") +
  ...
  lims(x = c(0, 10),
       y = c(0, 10)) +
  labs(x = expression("Chl"[SEAPOINT] ~ "(µg/L)"),
       y = expression("Chl"[AQUAFLUOR] ~ "(µg/L, Chl)"),
       color = "Depth") +
  theme_bw() +
  theme(text = element_text(size = 32),
...
f3 <- cc_long %>% 
  ...
  geom_smooth(aes(x = s_1, y = mean_af_chl),
              method = "lm", color = "black", fill = "black",
              formula = y ~ x,
              fullrange = F, se = T, size = 1, alpha = 0.3) +
  geom_point(aes(x = s_1, y = mean_af_chl),
              alpha = 0.8, size = 4, stroke = 1.5) +
  stat_poly_eq(aes(x = s_1, y = mean_af_chl,
               label = paste(..eq.label.., ..rr.label.., sep = "~~")),
               formula = formula_2, parse = TRUE, size = 8,
               color = "black", label.y.npc = 0.99, label.x.npc = 0.2) +
  geom_abline(slope = 1, linetype = "dashed") +
  ggsci::scale_fill_aaas() +
  ggsci::scale_color_aaas() +
  lims(x = c(0, 110),
       y = c(0, 110)) +
  labs(x = expression("Chl"[SEAPOINT] ~ "(µg/L)"),
       y = expression("Chl"[AQUAFLUOR] ~ "(µg/L, RE)"),
       color = "Depth") +
  theme_bw() +
  theme(text = element_text(size = 32),
        axis.text = element_text(colour = "black"),
...
ggsave(here("figures", "scatter_comprisons.png"), fig,
        width = 20, height = 6, dpi = 300) 
```

```{r}
ss_long <- ss %>%
  pivot_longer(c(chl:perc_change), names_to = "type", values_to = "value")  

ss_long %>% 
  filter(type == "perc_change" & !id == "post-cali") %>% 
  ggplot(aes(x = as.factor(id), y = value)) +
  geom_point(size = 4) +
  geom_hline(yintercept = 10, color = "red", linetype = "dashed", size = 1.5) +
  geom_hline(yintercept = -10, color = "red", linetype = "dashed", size = 1.5) +
  labs(y = "Solid Standard (% Change)") +
  theme_bw() +
  theme(text = element_text(size = 32),
        axis.text = element_text(colour = "black"),
        axis.title.x = element_blank())

ggsave(here("figures", "ss_chl.png"),
